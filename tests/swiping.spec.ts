...
test.describe('Movie Swiping', () => {
 ...
 expect(newTitle).not.toBeNull();
  ...
  test('"They liked this!" pill shows when partner already right-swiped', async ({ browser }) => {
  ...
  const pillVisible = await partner1Page.locator('text="👍 They liked this"').isVisible().catch(() => false);

  if (pillVisible) {
  await expect(partner1Page.locator('text="👍 They liked this"')).toBeVisible();
  } else {
  test.skip(true, 'Partner did not see the same movie');
  }

  await partner1Context.close();
  await partner2Context.close();
  });
