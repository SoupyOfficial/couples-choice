...
import { loginAs, swipeRight, swipeLeft } from './helpers';
...
test.describe('Movie Swiping', () => {
  ...
  test('"They liked this!" pill shows when partner already right-swiped', async ({ page }) => {
    ...
    if (pillVisible) {
      ...
      test.skip(true, 'Partner 2 did not see the same movie (expected with parallel tests)');
    }
  });
